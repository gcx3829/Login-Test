...
public class Loan {
	...
	public int verify() {
		...
		this.book.setRentedBy(this.renter.getUsername());
		if(changeStatus == 0) { //change status failed
			return 3; //an unspecified problem occurs
		}
		String checkoutDate = LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
		this.setCheckOutDate(checkoutDate); 
		this.book.setCheckOutDate(checkoutDate);
		
		String ReturnDate = LocalDateTime.now().plusDays(14).format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
		this.setReturnDate(ReturnDate);
		this.book.setReturnByDate(ReturnDate);;
