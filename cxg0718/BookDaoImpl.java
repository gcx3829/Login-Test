...
public int addBook(Book t) {
	...
	//@Override
		...
		//NEW CODEN ADDED FOR PROCESS RETURN FUNCTION
		...
		//NEW CODE ADDED FOR EDIT BOOK FUNCTION
		public int update(Book book, String oldISBN) {
			...
			try{
				...
				ps.clearBatch();
				//IF TITLE ALREADY EXISTS, UODATE IT
				if (temp == 1) {
					ps = conn.prepareStatement("update titlelist set Title = ?, Author = ?, Genre = ?, Edition = ? where ISBN = ?");
					...
					//TITLE NOT EXISTS, ADD NEW TITLE
