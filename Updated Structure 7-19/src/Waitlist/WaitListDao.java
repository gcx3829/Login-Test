...
public interface WaitListDao {
	// *** HOUYI's CODE *** //
	public int add(Book b, User u); // for when user first signs up for waitlist
	//note that in table, waitlist has fields for book ISBN, username, position, and rentByDate in waitlist in table
	public int deleteUser(Book b, User u); //for when user has moved to top of waitlist, and then sucessfully
	//checked out book
