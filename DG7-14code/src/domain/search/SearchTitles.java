...
public class SearchTitles implements Search{
	...
	SearchTitles(String ISBN, String Title, String Author, String genre, String edition) {
		//
	...
	private String displayBooks(List<Book> books) {
		...
		if (books.size()==0) { //no books returned; return warning message about search instead
			...
			displayTable.append("<tr><th>ISBN</th><th>Title</th><th>Author</th><th>Category</th><th>Edition</th><th>Status</th>"); //set up first line of table
			String last = "";
			for (int i=0;i<books.size(); i++) {
				if (last.compareTo(books.get(i).getISBN())!=0) { //does not print same title twice
					...
					if (Integer.toString(bookDao.findStatus(books.get(i))).compareTo("1")==0) {
						statusText = "Available";
					} else if(Integer.toString(bookDao.findStatus(books.get(i))).compareTo("5")==0) {
