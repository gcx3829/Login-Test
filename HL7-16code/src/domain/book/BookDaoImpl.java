...
public class BookDaoImpl implements BookDao {
	...
	public Book getFirstBook(Book book) {
		...
		try{
			conn = db.getConnection();
			ps =conn.prepareStatement("select inventoryID from collection where ISBN = ? and Status = 1");
			ps.setString(1, book.getISBN());
			ResultSet rs = ps.executeQuery();
			while(rs.next()){
	...
	public int changeStatus(Book book, int newStatus) {
		...
		try{
			conn = db.getConnection();
			ps =conn.prepareStatement("update collection set Status = ? where ISBN = ? and inventoryID = ?");
