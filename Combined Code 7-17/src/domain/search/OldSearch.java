...
public class OldSearch {
	...
	public List<Book> search(Book b) {
		...
		temp = bookDao.SearchTitleFields("Category", b.getGenre());
		books.retainAll(temp);
		
			temp = bookDao.SearchTitleFields("Edition", b.getEdition());
			books.retainAll(temp);
		
		if (b.getInventoryID().length()>0) { //need to check for all fields which an be Null
			temp = bookDao.SearchCopyFields("CopyID", b.getInventoryID());
			books.retainAll(temp);
		}
	...
	public String getCopyID(String ISBN, String TargetStatus) {
		...
		if (books.size()>0)  { //get first copy that meets requirements
			message = books.get(0).getInventoryID();
		}
		return message;
	}
